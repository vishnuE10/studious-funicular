...
const Editor = () => {
        ...
        customButton.addEventListener('click', () => {
          ...
          quill.on('text-change', (delta, oldDelta, source) => {
            if (source === 'user') {
              const selection = quill.getSelection();
              if (selection) {
                const content = quill.getText(selection.index, selection.length).trim();
                const lastWord = content.split(' ').pop();
                console.log(lastWord);
                const matchedTitle = fuzzyMatchLastWord(lastWord, predefinedTitles);

                if (matchedTitle) {
                  quill.deleteText(selection.index - lastWord.length, lastWord.length);
                  quill.insertText(
                    selection.index,
                    matchedTitle,
  ...
  function fuzzyMatchLastWord(lastWord, predefinedTitles) {
    ...
    for (const title of predefinedTitles) {
      const score = stringSimilarity.compareTwoStrings(lastWord.toLowerCase(), title.toLowerCase());
      if (score > bestScore) {
        bestMatch = title;
        bestScore = score;
      }
